...
namespace LightSwitchApplication.PrintTemplates.Interpost
{
    public partial class InterpostPrint : DevExpress.XtraReports.UI.LightSwitchReport
    {
        public InterpostPrint()
        {
            InitializeComponent();
        }

        private void InterpostPrint_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
        ...
        {
            VouchersViewDouble v = (VouchersViewDouble)GetCurrentRow();
        ...
        private void InterpostPrint_DataSourceDemanded(object sender, EventArgs e)
        {
            if (this.Parameters.Count > 0)
            {
                string voucherIDs = (string)Parameters[0].Value;
                ...
                var cData = (from i in ServerApplicationContext.Current.DataWorkspace.CourierAzureData1.VouchersViewDoubles where lstidS.Contains(i.VoucherID) select i).OrderBy(i=>i.VoucherID);
